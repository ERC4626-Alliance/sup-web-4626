...
const Resources = () => {
  ...
  return (<>
    <div className="bg-gradient-to-t from-pink-100 pt-8 pb-16 text-center" id="resources">
      <Container>
        <>
          <AtomTitle alignText="center" subtitle="Explore relevant resources">ERC4626 RESOURCES</AtomTitle>
          <ul className="gap-6 flex flex-wrap flex-row justify-center">
            {linksResources.repositories.map((item) => (<li key={item.name}>
              <a
                href={item.href}
                target="_blank"
                rel="noreferrer noopener group"
                className={`inline-flex items-center pl-2.5 pr-4 py-3.5 shadow-sm border border-pink-600/10 rounded-lg whitespace-nowrap text-sm md:text-base ${item.highlight ?
                                                                                                           "font-bold" :
                                                                                                           "font-medium"} text-pink-700 hover:underline underline-offset-4 group bg-white`}
              >
                <svg fill="currentColor" viewBox="0 0 24 24"
                     className={`-ml-0.5 mr-1.5 md:h-8 md:w-8 w-4 h-4 ${item.highlight ? "opacity-75" : "opacity-25"} md:group-hover:opacity-100`}>
