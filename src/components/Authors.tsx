  ...
  return (<div className="py-16 bg-gradient-to-t from-pink-50 text-center" id="authors">
      <Container>
        ...
        <div
          ...
          {authors.map((author: { name: string; twitter: string; githubAvatar?: string; }, index) => (
            <>
              <a href={`https://twitter.com/${author.twitter}`} target="_blank" rel="nofollow noreferrer" key={`author-${index}`}
                ...
                <div className="relative rounded-full overflow-hidden w-10 h-10  ring-2 ring-transparent md:group-hover:ring-pink-700">
                  <img src={author.githubAvatar} alt="" className="bg-gray-200 w-full h-full" loading="lazy"/>
              ...
              <span className="hidden lg:inline">{index < authors.length - 2 && ", "}{index == authors.length - 2 && ", and "}</span>
            </>
          ))}
        </div>
      </Container>
